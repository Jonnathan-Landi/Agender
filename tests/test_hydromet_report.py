...
class HydrometReportIntegrationTests(TestCase):
    ...
    def test_uv_risk_is_detected_from_the_inserted_chart(self) -> None:
        ...
        self.assertIn("color.chroma >= 0.22", self.feature)
        self.assertIn("color.saturation >= 0.2", self.feature)
    ...
    def test_one_datetime_control_updates_every_report_page(self) -> None:
        ...
        self.assertIn("reportDate.setMinutes(0, 0, 0)", self.feature)
        self.assertNotIn("reportDate.setSeconds(0, 0)", self.feature)
        self.assertIn('querySelectorAll("[data-hydromet-rain-end-time]")', self.feature)
        self.assertIn("input.value = inputTime", self.feature)
    ...
    def test_rainfall_table_has_an_independent_date_seeded_from_report_date(self) -> None:
        ...
        self.assertEqual(0, graphics.count(">0</td>"))
        self.assertEqual(21, graphics.count(">0.00</td>"))
    ...
    def test_temperature_execution_generates_and_places_the_map_in_design(self) -> None:
        ...
        self.assertIn("function syncTemperatureTimes", self.feature)
        self.assertIn("date.getTime() + 60 * 60 * 1000", self.feature)
        self.assertIn("syncTemperatureTimes(card, temperatureReportDate)", self.feature)
