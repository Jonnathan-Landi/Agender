...
from .queries import build_export_query, execute_query, query_data
...
app = FastAPI(title="Agender Viewer API", version="1.20.5")
...
def export_data(payload: ExportRequest) -> Any:
    ...
    sql, params, headers = build_export_query(payload, meta, parquet_path(payload.session_id))
    rows = execute_query(sql, params, WORKER_THREADS)
    if not rows:
...
def get_data(
    ...
    return query_data(
        ...
        meta=meta,
        dataset_path=parquet_path(session_id),
        worker_threads=WORKER_THREADS,
    )
